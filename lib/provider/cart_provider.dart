 ...
 CartProvider(){
  ...
  // read cart products
    ...
    _productSubscription = DbService().searchProducts(uids).listen((snapshot) {
      List<AnimalModel> productsData =
          AnimalModel.fromJsonList(snapshot.docs) as List<AnimalModel>;
  ...
  void addCost(List<AnimalModel> products, List<CartModel> carts) {
    totalCost = 0;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      for (int i = 0; i < carts.length; i++) {
        totalCost += carts[i].quantity * products[i].amount;
      }
      notifyListeners();
    });
  }
